...
def fftToAbs(fftData):
    return np.log(np.abs(fftData) + 1.0)
...
def fftToData(fftData, rawIntensity):
    ...
    for i in np.arange(0, np.size(fftData, 0), 1):
        reconstructData[i] = (
            reconstructData[i] * rawIntensity[i] / np.mean(reconstructData[i])
        )
...
def angles2complex(normalArray):
    ...
    for i in range(len(newArray)):
        newArray[i] = normalArray[i] * (
            complex(cos(normalArray[i + len(newArray)]))
            + (1j) * sin(normalArray[i + len(newArray)])
        )
    return newArray
...
def simplexMethod(
    loss_function, alpha, startValues, para_start, para_stop, *loss_function_para
):
    N = len(startValues)
    ...
    for i in range(N + 1):
        ...
        for j in range(para_start, para_stop):
            randomArray[j] = np.random.uniform(1.0 / alpha, alpha)
    ...
    for iter in range(200):
        ...
        pointCOM /= N
        newPoint = 2.0 * pointCOM - pointsMatrix[max_loss]
        if loss_function(newPoint, loss_function_para) > lossValues[max_loss]:
            pointsMatrix[max_loss] = pointCOM + 0.7 * (
                pointCOM - pointsMatrix[max_loss]
            )
            lossValues[max_loss] = loss_function(
                pointsMatrix[max_loss], loss_function_para
            )
        else:
            pointsMatrix[max_loss] = pointCOM + 1.2 * (
                pointCOM - pointsMatrix[max_loss]
            )
            lossValues[max_loss] = loss_function(
                pointsMatrix[max_loss], loss_function_para
                    ...
                    * (
                        ...
                        * (
                            cos(
                                ...
                                * pi
                                * (
                                    (XYPoses[index, 0] / xLen) * m
                                    + (XYPoses[index, 1] / yLen) * n
                                )
                            )
                            + (1.0j)
                            * sin(
...
def meshLoss(fourierPoints_compressed_with_XyPos, allPara):
    ...
    fourierPoints_compressed = fourierPoints_compressed_with_XyPos[
        : len(fourierPoints_compressed_with_XyPos) - 4
    ].copy()
    XYPoses = fourierPoints_compressed_with_XyPos[
        len(fourierPoints_compressed_with_XyPos) - 4 :
    ].copy()
...
def meshLoss4(fourierPoints_compressed_with_XyPos, allPara):
    ...
    XYPos_1st = fourierPoints_compressed_with_XyPos[
        len(fourierPoints_compressed_with_XyPos) - 4 :
    ].copy()
...
def autoRemoveFourierPeaks(image):
    ...
    for peak_index in range(1, 9):
        ...
        for delta_x in range(3, 20):
            if (
                np.sum(absFftData[x_center + delta_x, y_center - 7 : y_center + 8])
                + np.sum(absFftData[x_center - delta_x, y_center - 7 : y_center + 8])
                < absFftData[x_center, y_center]
            ):
                break
        for delta_y in range(3, 20):
            if (
                np.sum(absFftData[x_center - 7 : x_center + 8, y_center + delta_y])
                + np.sum(absFftData[x_center - 7 : x_center + 8, y_center - delta_y])
                < absFftData[x_center, y_center]
            ):
                break
...
def optimizeMeshFromData(meshData, rawImage):
    ...
    if sum > 0.0:
        ...
        recreatedMesh = createMeshFromPeaks(
            optimized_para[: len(optimized_para) - 4],
            optimized_para[len(optimized_para) - 4 :],
            len(meshData),
            len(meshData[0]),
        )
        recreatedMesh = (
            recreatedMesh * np.mean(rawImage) / np.mean(recreatedMesh * rawImage)
        )
            ...
            for j in range(N):
                ...
                for iter1 in range(size):
                    for iter2 in range(size):
                        imageArray[N * i + j, iter1, iter2] = inputImage[
                            startX + iter1, startY + iter2
                        ]
    else:
    ...
    if M * N != 1:
        for i in range(M):
            for j in range(N):
                ...
                for iter1 in range(size):
                    for iter2 in range(size):
                        largeImage[startX + iter1, startY + iter2] += (
                            imageArray[N * i + j, iter1, iter2] * imageWeight[N * i + j]
                        )
                        datacount[startX + iter1, startY + iter2] += imageWeight[
                            N * i + j
                        ]
        ...
        if (xRange[1] - xRange[0]) > (yRange[1] - yRange[0]):
            for i in np.arange(xRange[0], xRange[1], 1):
                for j in np.arange(yRange[0], yRange[1], 1):
                    fftData[index, i, j] = fftData[index, i, yRange[0]] + (
                        fftData[index, i, yRange[1]] - fftData[index, i, yRange[0]]
                    ) * (j - yRange[0]) / (yRange[1] - yRange[0])
                    fftData[
                        index, np.size(fftData, 1) - 1 - i, np.size(fftData, 2) - 1 - j
                    ] = fftData[index, i, j]
        else:
            for i in np.arange(yRange[0], yRange[1], 1):
                for j in np.arange(xRange[0], xRange[1], 1):
                    fftData[index, j, i] = fftData[index, xRange[0], i] + (
                        fftData[index, xRange[1], i] - fftData[index, xRange[0], i]
                    ) * (j - xRange[0]) / (xRange[1] - xRange[0])
                    fftData[
                        index, np.size(fftData, 1) - 1 - j, np.size(fftData, 2) - 1 - i
                    ] = fftData[index, j, i]
            ...
            for i in np.arange(xRange[0], xRange[1], 1):
                for j in np.arange(yRange[0], yRange[1], 1):
                    if (
                        ...
                        smallfftData[i, j] = smallfftData[i, yRange[0]] + (
                            smallfftData[i, yRange[1]] - smallfftData[i, yRange[0]]
                        ) * (j - yRange[0]) / (yRange[1] - yRange[0])
                        smallfftData[blockSize - 1 - i, blockSize - 1 - j] = (
                            smallfftData[i, j]
                        )
...
def drawRect(data, paraList=[]):
    ...
    for index in np.arange(0, np.size(data, 0), 1):
        ...
        for i in np.arange(xRange[0], xRange[1] + 1, 1):
            ...
            data[index, np.size(data, 1) - 1 - i, np.size(data, 2) - 1 - yRange[0]] = (
                maxI
            )
            data[
                index, np.size(data, 1) - 1 - i, np.size(data, 2) - 1 - yRange[0] - 1
            ] = maxI
            data[index, np.size(data, 1) - 1 - i, np.size(data, 2) - 1 - yRange[1]] = (
                maxI
            )
            data[
                index, np.size(data, 1) - 1 - i, np.size(data, 2) - 1 - yRange[1] + 1
            ] = maxI
        for j in np.arange(yRange[0], yRange[1] + 1, 1):
            data[index, np.size(data, 1) - 1 - xRange[0], np.size(data, 2) - 1 - j] = (
                maxI
            )
            data[index, np.size(data, 1) - 1 - xRange[1], np.size(data, 2) - 1 - j] = (
                maxI
            )
            data[
                index, np.size(data, 1) - 1 - xRange[0] - 1, np.size(data, 2) - 1 - j
            ] = maxI
            data[
                index, np.size(data, 1) - 1 - xRange[1] + 1, np.size(data, 2) - 1 - j
            ] = maxI

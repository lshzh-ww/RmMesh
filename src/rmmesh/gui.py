...
class MyWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.leftGraphWidget = rmmesh.custom_widgets.plot.myImageView(
            topLeft, view=pg.PlotItem()
        )
        self.leftGraphWidget.show()
        self.rightGraphWidget = rmmesh.custom_widgets.plot.myImageView(
            topRight, view=pg.PlotItem()
        )
    ...
    def cropData(self):
        ...
        self.rawData = self.rawData[
            :, int(paraList[0]) : int(paraList[1]), int(paraList[2]) : int(paraList[3])
        ]
        dataShape = np.shape(self.rawData)
        print(dataShape)
        self.displayData = self.rawData.copy()
    ...
    def saveFileDiag(self):
        ...
        print(saveFilename)
        rmmesh.file_io.load_data.saveFile(
            self, self.fname[0], saveFilename[0], self.displayData
        )
    ...
    def restoreCurvatureFunc(self):
        ...
        for i in np.arange(0, np.size(self.displayData, 0), 1):
            self.restoredData[i] = rmmesh.comp_func.fft.restoreCurvature2D(
                self.displayData[i], self.radius
            )
        self.displayData = self.restoredData
        self.leftGraphWidget.setImage(self.displayData)
        self.finalMesh = np.zeros(np.shape(self.meshPattern))
        self.finalMesh[0] = rmmesh.comp_func.fft.restoreCurvature2D(
            self.meshPattern[0], self.radius
        )
    ...
    def startRemoveMesh(self):
        if self.leftConfCheckBox.isChecked():
            ...
            self.avgRawData = np.zeros(
                (1, np.shape(self.rawData[0])[0], np.shape(self.rawData[0])[1])
            )
            for i in np.arange(0, np.size(self.rawData, 0), 1):
            ...
            for image_index in range(len(self.rawData)):
                newImage = rmmesh.comp_func.fft.autoRemoveFourierPeaks(
                    self.rawData[image_index]
                )
                # newImage=rmmesh.CompFunc.fft.autoRemoveFourierPeaks(newImage)
                self.meshPattern = rmmesh.comp_func.fft.getRatio(
                    self.rawData[image_index : image_index + 1, :, :],
                    np.reshape(newImage, (1, len(newImage), len(newImage[0]))),
                )
                ...
                self.smallImageArray = rmmesh.comp_func.fft.splitLargeImage(
                    self.meshPattern[0], M, N, blockSize
                )
                self.avgScanImageArray = rmmesh.comp_func.fft.splitLargeImage(
                    self.rawData[image_index], M, N, blockSize
                )
                ...
                if __name__ == "__main__":
                    with Pool(processes=CPU_NUM) as pool:
                        ...
                        while (index + CPU_NUM) < M * N:
                            m_Results = [
                                pool.apply_async(
                                    rmmesh.comp_func.fft.optimizeMeshFromData,
                                    args=(
                                        self.smallImageArray[i],
                                        self.avgScanImageArray[i],
                                    ),
                                )
                                for i in range(index, index + CPU_NUM)
                            ]
                            for i in range(CPU_NUM):
                                (
                                    self.optimizedMeshArray[index + i],
                                    self.optimizedMeshLoss[index + i],
                                ) = m_Results[i].get()
                ...
                self.meshPattern[0] = rmmesh.comp_func.fft.constructLargeImage(
                    self.optimizedMeshArray,
                    1.0 / self.optimizedMeshLoss,
                    M,
                    N,
                    blockSize,
                    len(self.rawData[0]),
                    len(self.rawData[0][0]),
                )
                self.displayData[image_index] = (
                    self.meshPattern[0] * self.rawData[image_index]
                )
    ...
    def leftSelectedRegion(self):
        ...
        self.absFftData = rmmesh.comp_func.fft.fftToAbs(self.fftData)
        self.rightGraphWidget.setImage(self.absFftData)
        self.displayData = rmmesh.comp_func.fft.fftToData(
            self.fftData, self.rawIntensity
        )
    ...
    def leftShowMesh(self):
        if self.leftDisplay == "ReconstructData":
            self.meshPattern = rmmesh.comp_func.fft.getRatio(
                self.avgRawData, self.displayData
            )
            self.leftGraphWidget.setImage(self.meshPattern)
            self.leftGraphWidget.setLevels(min=0, max=3.0)
    ...
    def rightFinish(self):
        ...
        for i in np.arange(0, np.size(self.rawData, 0), 1):
            self.fixedData[i] = rmmesh.comp_func.fft.fixCurvature2D(
                self.rawData[i], self.radius
            )
    ...
    def testFunc(self):
        ...
        self.avgScanImageArray = rmmesh.comp_func.fft.splitLargeImage(
            self.avgRawData[0], M, N, blockSize
        )
        self.optimizedMeshArray = np.zeros(self.smallImageArray.shape)
        self.optimizedMeshLoss = np.zeros(M * N)
